...
def _read_session_id(binding: Any) -> str | None:
    """Read a session ID from the runtime binding response."""
...
async def _session_options_for_id(
    session_store: Any,
    session_id: str,
) -> tuple[str | None, str | None]:
    """Choose session_id for a new transcript or resume for an existing one."""
...
async def resolve_claude_session_binding(
    ...
    """Map a Conversation ID to Claude SDK session options.

    Updated runtimes persist a stable mapping and support arbitrary IDs. Older
    runtimes retain the UUID-only fallback for compatibility.
    """
    if not conversation_id:
        return None, None

    binding_method = getattr(store_adapter, "claude_session_binding", None)
    if callable(binding_method):
        try:
            session_id = _read_session_id(await binding_method(conversation_id))
            if session_id:
                return await _session_options_for_id(session_store, session_id)
            logger.log(f"[session] runtime returned no binding for conversation_id={conversation_id!r}")
        except Exception as e:
            logger.error(f"[session] failed to resolve runtime Claude session binding: {e}")
        return None, None

    # Compatibility path for runtimes predating claude_session_binding(). It is
    # intentionally UUID-only; arbitrary Conversation IDs require the new API.
    legacy_session_id = _normalize_uuid(conversation_id)
    if not legacy_session_id:
        logger.log(f"[session] skip legacy SDK session binding: invalid conversation_id={conversation_id!r}")
        return None, None
    return await _session_options_for_id(session_store, legacy_session_id)
...
async def handler(ctx: Any) -> AsyncGenerator[str, None]:
    ...
    sdk_session_id, sdk_resume = await resolve_claude_session_binding(store_adapter, session_store, cid)
